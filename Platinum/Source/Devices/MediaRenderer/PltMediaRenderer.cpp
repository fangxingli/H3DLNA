...
PLT_MediaRenderer::SetupServices()
{
...

    {
        ...
        service->SetStateVariable("CurrentTransportActions", "Play,Pause,Stop,Seek,Previous,Next");
...

    {
        ...
        service->SetStateVariable("Volume", "10");
...
#define print_action(__action) do \
					printf("%s\n",__action.GetChars());	\
				while(0)
    ...
    if (name.Compare("GetVolume", true) == 0) {
	  print_action(name);
          return OnGetVolume(action);
    }
    if (name.Compare("GetMute", true) == 0) {
	  print_action(name);
          return OnGetMute(action);
    } 
#endif
...
/*----------------------------------------------------------------------
|   By Lee
|   PLT_MediaRenderer::OnGetPositionInfo
+---------------------------------------------------------------------*/
#if 1
NPT_Result 
...
#if 1
/*----------------------------------------------------------------------
|   PLT_MediaRenderer::OnGetVolume
+---------------------------------------------------------------------*/
NPT_Result
PLT_MediaRenderer::OnGetVolume(PLT_ActionReference& action)
{
    return NPT_ERROR_NOT_IMPLEMENTED;
}

/*----------------------------------------------------------------------
|   PLT_MediaRenderer::OnGetMute
+---------------------------------------------------------------------*/
NPT_Result
PLT_MediaRenderer::OnGetMute(PLT_ActionReference& action)
{
    return NPT_ERROR_NOT_IMPLEMENTED;
}
#endif
