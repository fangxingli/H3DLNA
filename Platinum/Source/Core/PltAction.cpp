...
{
	char buf[64],*tail = NULL, *cur = NULL;
    if (m_ErrorCode) {
...
#if 0
	if( strstr((char *)str, "GetCurrentTransportActions") )
		printf("RESPONSE: %s\n", str.GetChars());
	else{
#endif
	cur = strstr((char *)str,"<CurrentTransportState>");
	if( cur ){
		tail = strstr(cur, "</CurrentTransportState>");
		tail += strlen("</CurrentTransportState>");
		memset(buf, 0, 64);
		strncpy(buf, cur, tail-cur);
		printf("RESPONSE: %s\n", buf);
	}
    return stream.Write((const char*)str, str.GetLength());
